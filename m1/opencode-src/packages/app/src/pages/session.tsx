...
import { BrowserPanel } from "@/pages/session/browser-panel" // GRAMSAI_BROWSER_TAB
...
export default function Page() {
  ...
  const onViewFile = createOpenSessionFileTab({
    normalizeTab: (tab) => file.tab(tab),
    openTab: tabs().open,
    pathFromTab: file.pathFromTab,
    loadFile: (p) => file.load(p),
    openReviewPanel: () => {
  ...
  const workspaceKey = createMemo(() => params.dir ?? "L3dvcmtzcGFjZQ")
  ...
  // GRAMSAI_BROWSER_TAB: render fn for the live browser panel (mirrors reviewPanel).
  const browserActive = () => activeTab() === "browser"
  const browserPanel = () => <BrowserPanel active={browserActive} />
      ...
      >
        ...
        >
          ...
          >
            <Switch>
              {/* GRAMSAI_BROWSER_MOBILE */}
              <Match when={params.id && store.mobileTab === "browser"}>
                <div class="relative h-full overflow-hidden">
                  <BrowserPanel active={() => store.mobileTab === "browser"} />
                </div>
              </Match>
        ...
        <SessionSidePanel
          ...
          browserActive={() => true /* GRAMSAI_BROWSER_GATE_FIX: tab always available; avoids activeTab<->gate cycle */}
