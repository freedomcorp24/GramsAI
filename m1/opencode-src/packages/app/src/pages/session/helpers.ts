...
export const createSessionTabs = (input: TabsInput) => {
  ...
  const browser = input.browser ?? (() => false) // GRAMSAI_BROWSER_TAB
  ...
  const openedTabs = createMemo(
    () => {
      ...
      return input
        ...
        .all()
        .flatMap((tab) => {
          if (tab === "context" || tab === "review" || tab === "browser" || tab === "files") return []
...
export const createOpenSessionFileTab = (input: {
  ...
  return (value: string) => {
    // GRAMSAI_BROWSER_CLICK: the browser tab is a pinned (non-file) tab. The
    // file-tab path below bails when pathFromTab() is empty, so handle it here:
    // open the side panel and make browser the active tab.
    if (value === "browser") {
      input.openReviewPanel()
      input.openTab("browser")
      input.setActive("browser")
      return
    }
