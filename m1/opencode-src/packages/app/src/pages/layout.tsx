...
import { PlanBanner } from "./layout/plan-banner"
...
export default function Layout(props: ParentProps) {
  ...
  const prefetchSession = (session: Session, priority: "high" | "low" = "low") => {
    // GRAMSAI_NO_BOOT_PREFETCH: never prefetch chat content in the background.
    // Only load a chat's messages when it is explicitly opened ("high").
  ...
  function openSettings(tab?: string) {
    ...
    void module.then((x) => {
      if (dialogDead || dialogRun !== run) return
      dialog.show(() => <x.DialogSettings initialTab={tab} />)
    })
  }
      ...
      fallback={
        <div class="relative bg-v2-background-bg-deep flex-1 min-h-0 min-w-0 flex flex-col select-none [&_input]:select-text [&_textarea]:select-text [&_[contenteditable]]:select-text">
          ...
          <div class="flex-1 min-h-0 min-w-0 flex">
            {/* Left chat nav (collapsible). Desktop pushes content; mobile is a drawer. */}
            <div
              classList={{
                "hidden md:block shrink-0 overflow-hidden border-r border-v2-border-border-muted": true,
                "transition-[width] duration-200 motion-reduce:transition-none": true,
              }}
              style={{ width: layout.sidebar.opened() ? "260px" : "0px" }}
            >
            ...
            <div class="md:hidden">
              ...
              <div
                classList={{
                  "fixed top-10 bottom-0 left-0 z-50 w-[280px] max-w-[85vw] overflow-hidden border-r border-v2-border-border-muted transition-transform duration-200 ease-out": true,
