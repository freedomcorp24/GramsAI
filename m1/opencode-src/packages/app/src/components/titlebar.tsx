            ...
            return (
              ...
              >
                <button
                  type="button"
                  onClick={() => {
                    if (window.matchMedia("(min-width: 768px)").matches) layout.sidebar.toggle()
                    else layout.mobileSidebar.toggle()
                  }}
                  aria-label="Toggle sidebar"
                  class="shrink-0 flex select-none items-center px-1 font-mono text-[22px] font-bold leading-none"
                >
                  <span class="text-[#3fb950] font-bold">gr</span>
                  <span class="text-v2-text-text-base">ams</span>
                </button>
        ...
        <Match when>
          ...
          >
            ...
            <div
              ...
              onMouseDown={drag}
            >
              <div id="opencode-titlebar-right" class="flex items-center gap-1 shrink-0 justify-end"><div id="gramsai-session-dots" class="flex items-center mr-1" /></div>
              <Show when={windows()}>
...
function TitlebarV2Right(props: { state: TitlebarV2RightState }) {
  return (
    <div class="flex shrink-0 items-center justify-end gap-0">
      ...
      <div id="opencode-titlebar-right" class="flex shrink-0 items-center justify-end gap-0"><div id="gramsai-session-dots" class="flex items-center mr-2" /></div>
    </div>
  )
}
...
function ProjectTabAvatar(props: { project?: LocalProject; directory: string; sessionId: string }) {
  ...
  return (
    <span
      ...
      title={displayName(props.project ?? { worktree: props.directory })}
    >
      <svg viewBox="0 0 24 24" fill="none" class="size-3.5">
        <path d="M12 2 3 7v10l9 5 9-5V7l-9-5Z" stroke="#3fb950" stroke-width="1.8" stroke-linejoin="round" />
