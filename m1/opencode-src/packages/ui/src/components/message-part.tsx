...
import { getDirectory as _getDirectory, getFilename, sanitizePath } from "@opencode-ai/core/util/path"  // GRAMSAI_MP_SANITIZE
...
function getDirectory(path: string | undefined) {
  const data = useData()
  return sanitizePath(relativizeProjectPath(_getDirectory(path), data.directory))
}
...
function contextToolTrigger(part: ToolPart, i18n: ReturnType<typeof useI18n>) {
  ...
  const path = sanitizePath(typeof input.path === "string" ? input.path : "/")
  const filePath = typeof input.filePath === "string" ? sanitizePath(input.filePath) : undefined
